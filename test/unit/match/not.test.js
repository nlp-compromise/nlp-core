...
test('not-basic :', function(t) {
  ...
  m = nlp('spencer is really cool').not('really');
  ...
  m = nlp('spencer is really cool').not('is really+');
  t.equal(m.out('text'), 'spencer cool', 'three-not');
  t.equal(m.length, 2, 'two-results');

  m = nlp('spencer is really cool. John is really nice.').not('really');
  t.equal(m.out('text'), 'spencer is cool. John is nice.', 'two-terms-matches');
  t.equal(m.length, 4, 'four-results');

  m = nlp('spencer is really cool. John is really nice.').not('pardon me, really');
